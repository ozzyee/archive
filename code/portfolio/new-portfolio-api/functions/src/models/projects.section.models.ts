...
export const getAllProjects = async () => {
   ...
   try {
      ...
      const querySnapshot = await db.collection("projects_section").get();
...
export const getProjectsById = async (id: string) => {
   ...
   try {
      const ref = await db.collection("projects_section").doc(id).get();
      const data = ref.data();
      _data = data;
...
export const addProjects = async (data: object) => {
   let message = "entry added successfully",
      _data,
      hasError = false;

   try {
      const entry = db.collection("projects_section").doc();
      const entryObject = {
         id: entry.id,
         ...data,
      };
...
export const updateProjects = async (data: object, id: string) => {
   let _data,
      statusCode = 200,
      status,
      hasError = false,
   ...
   try {
      const entry = db.collection("projects_section").doc(id);
