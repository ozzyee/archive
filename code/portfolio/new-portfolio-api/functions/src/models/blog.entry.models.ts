...
export const getAllEntries = async () => {
   ...
   try {
      ...
      const querySnapshot = await db.collection("blog_entries").get();
...
export const getEntriesById = async (id: string) => {
   ...
   try {
      const ref = await db.collection("blog_entries").doc(id).get();
      const data = ref.data();
      _data = data;
...
export const addEntry = async (data: object) => {
   let message = "entry added successfully",
      _data,
      hasError = false;

   try {
      const entry = db.collection("blog_entries").doc();
      const entryObject = {
         id: entry.id,
         ...data,
      };
...
export const updateEntry = async (data: object, id: string) => {
   let _data,
      statusCode = 200,
      status,
      hasError = false,
   ...
   try {
      const entry = db.collection("blog_entries").doc(id);
