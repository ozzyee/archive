...
export const getAllSkills = async () => {
   ...
   try {
      ...
      const querySnapshot = await db.collection("skills_section").get();
...
export const getSkillsById = async (id: string) => {
   ...
   try {
      const ref = await db.collection("skills_section").doc(id).get();
      const data = ref.data();
      _data = data;
...
export const addSkills = async (data: object) => {
   let message = "entry added successfully",
      _data,
      hasError = false;

   try {
      const entry = db.collection("skills_section").doc();
      const entryObject = {
         id: entry.id,
         ...data,
      };
...
export const updateSkills = async (data: object, id: string) => {
   let _data,
      statusCode = 200,
      status,
      hasError = false,
   ...
   try {
      const entry = db.collection("skills_section").doc(id);
